...
> `가상스레드 방식`과 `논블록킹 방식(DB는 블록킹)`이 높은 성능을 나타내었으며, `가상스레드 방식`이 더 많은 요청에서 좋은 성능을 보여주었습니다.

## DB 쓰기 API
DB 쓰기가 포함된 API의 성능 측정 결과 입니다.
> 후반부로 갈 수록 `가상스레드 방식`이 좋은 성능을 나타내었으며 그 다음으로  `논블록킹 방식(논 블록킹 DB)`이 높은 성능을 보여주었습니다.
