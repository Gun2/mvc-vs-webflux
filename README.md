...
<br/>※ webflux도 Thread.sleep(x) 메서드를 사용하였으며 이벤트루프 스레드에서 실행됩니다.
<br/>※ 설정은 모두 기본값을 사용하였습니다. (web flux의 경우 최소 수치인 4개의 이벤트루프 스레드가 할당되었습니다.)
...
# 결론
기존의 멀티 스레드 방식은 CPU 바운드 외에는 가장 낮은 성능을 보여주었으며, 가상 스레드 방식은 다양한 측정 범위에서 상위권을 유지하였습니다.
논블로킹 방식의 경우 가벼운 기능의 API에서 높은 성능을 보여주었습니다.

|                | 기존 스레드 | 가상 스레드 | webflux(논블로킹) |
|----------------|--------|--------|---------------|
| CPU 바운드        | 우수     | 낮음     | 낮음            |
| I/O 바운드        | 낮음     | 우수     | 우수            |
| 컨텍스트 스위칭 발생 빈도 | 높음     | 낮음     | 낮음            |
